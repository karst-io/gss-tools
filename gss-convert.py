...
parser = argparse.ArgumentParser(description='convert gss data format to karst data format')

parser.add_argument('gssfile', action="store")
    ...
    if line [0:3] == "  G":
        ...
        if record_type == 'E':
            ...
            if os.path.exists(cave_file_path) == True:
                ...
                if length != '':
                    length = int(length)
                    cave_obj['length'] = length

                if depth != '':
                    depth = int(depth)
                    cave_obj['depth'] = depth
            ...
            fptr = open(cave_file_path, 'r')
            cave_obj = json.loads(fptr.read())
            if 'alternate_names' in cave_obj:
                cave_obj['alternate_names'].append(name)
            else:
                cave_obj['alternate_names'] = [name]
            fptr = open(cave_file_path,'w')
            fptr.write(json.dumps(cave_obj, indent=4))
            fptr.close()
        else:
